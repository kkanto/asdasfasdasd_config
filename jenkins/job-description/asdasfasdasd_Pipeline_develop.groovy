...
def job_config = [
        job: [
                name: "asdasfasdasd_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'asdasfasdasd_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/asdasfasdasd_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.asdasfasdasd.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "asdasfasdasd", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("asdasfasdasd_Pipeline_develop");

job.with {

    authenticationToken('hRVoWMGMkVhWJ6hJbq0u5lFsdXQBmjjx')
